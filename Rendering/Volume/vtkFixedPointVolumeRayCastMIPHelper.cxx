...
// index to lookup the final color/opacity.
template <class T>
void vqFixedPointAIPHelperGenerateImageOneNN(T* data,
    int threadID,
    int threadCount,
    vtkFixedPointVolumeRayCastMapper* mapper,
    vtkVolume* vtkNotUsed(vol))
...
// as the color directly (four component data). Lookup alpha off the last component.
template <class T>
void vqFixedPointAIPHelperGenerateImageDependentNN(
    T* data,
    int threadID,
    int threadCount,
    vtkFixedPointVolumeRayCastMapper* mapper,
    vtkVolume* vtkNotUsed(vol))
{
    ...
    if (numSamples != 1)
    {
        ...
        aveIdx[components - 1] = (aveIdx[components - 1] + shift[components - 1]) * scale[components - 1];

        VTKKWRCHelper_LookupDependentColorUS(colorTable[0], scalarOpacityTable[0],
            aveIdx, components, imagePtr);
    }
    else
    {
        imagePtr[0] = imagePtr[1] = imagePtr[2] = imagePtr[3] = 0;
...
// the ray, and then use this to look up a final color.
template <class T>
void vqFixedPointAIPHelperGenerateImageOneSimpleTrilin(
    T* dataPtr,
    int threadID,
    int threadCount,
    vtkFixedPointVolumeRayCastMapper* mapper,
...
// final color.
template <class T>
void vqFixedPointAIPHelperGenerateImageOneTrilin(
    T* dataPtr,
    int threadID,
    int threadCount,
    vtkFixedPointVolumeRayCastMapper* mapper,
...
// higher than some threshold).
template <class T>
void vqFixedPointAIPHelperGenerateImageDependentTrilin(
    T* dataPtr,
    int threadID,
    int threadCount,
    vtkFixedPointVolumeRayCastMapper* mapper,
...
// them according to the component weights.
template <class T>
void vqFixedPointAIPHelperGenerateImageIndependentTrilin(
    T* dataPtr,
    int threadID,
    int threadCount,
    vtkFixedPointVolumeRayCastMapper* mapper,
    ...
    vtkFixedPointVolumeRayCastMapper* mapper)
{
    ...
    if (mapper->ShouldUseNearestNeighborInterpolation(vol))
    {
        ...
        if (mapper->GetCurrentScalars()->GetNumberOfComponents() == 1)
        {
            ...
            {
                vtkTemplateMacro(
                    vqFixedPointAIPHelperGenerateImageOneNN(
                        static_cast<VTK_TT*>(dataPtr),
                        threadID, threadCount, mapper, vol));
            }
        }
        ...
        else if (vol->GetProperty()->GetIndependentComponents())
        {
            ...
            {
                vtkTemplateMacro(
                    vqFixedPointAIPHelperGenerateImageIndependentNN(
                        static_cast<VTK_TT*>(dataPtr),
                        threadID, threadCount, mapper, vol));
            }
        }
        ...
        else
        {
            ...
            {
                vtkTemplateMacro(
                    vqFixedPointAIPHelperGenerateImageDependentNN(
                        static_cast<VTK_TT*>(dataPtr),
                        threadID, threadCount, mapper, vol));
            }
        }
    ...
    else
    {
        ...
        if (mapper->GetCurrentScalars()->GetNumberOfComponents() == 1)
        {
                ...
                mapper->GetTableShift()[0] == 0.0)
            {
                ...
                {
                    vtkTemplateMacro(
                        vqFixedPointAIPHelperGenerateImageOneSimpleTrilin(
                            static_cast<VTK_TT*>(dataPtr),
                            threadID, threadCount, mapper, vol));
                }
            }
            ...
            else
            {
                ...
                {
                    vtkTemplateMacro(
                        vqFixedPointAIPHelperGenerateImageOneTrilin(
                            static_cast<VTK_TT*>(dataPtr),
                            threadID, threadCount, mapper, vol));
                }
            }
        ...
        else if (vol->GetProperty()->GetIndependentComponents())
        {
            ...
            {
                vtkTemplateMacro(
                    vqFixedPointAIPHelperGenerateImageIndependentTrilin(
                        static_cast<VTK_TT*>(dataPtr),
                        threadID, threadCount, mapper, vol));
            }
        }
        ...
        else
        {
            ...
            {
                vtkTemplateMacro(
                    vqFixedPointAIPHelperGenerateImageDependentTrilin(
                        static_cast<VTK_TT*>(dataPtr),
                        threadID, threadCount, mapper, vol));
            }
        }
        ...
        for(int i = -Smoothness; i <= Smoothness; i++) { \
            for(int j = -Smoothness; j <= Smoothness; j++) { \
                for(int k = -Smoothness; k <= Smoothness; k++) { \
                    ...
                    unsigned int pos1[3] = {Pos[0] + i * 0x8000, Pos[1] + j* 0x8000, Pos[2] + k * 0x8000}; \
...
#define VTKVRCHelper_ComputeGradientNorm(Event, Smoothness ) \
    ...
    if(sdPos[0] <= (direction[0] + 1) || sdPos[1] <= (direction[1] + 1) || sdPos[2] <= (direction[2] + 1) || \
        sdPos[0] >= (dim[0]- (direction[0] + 1)) || sdPos[1] >= (dim[1] - (direction[0] + 1)) || sdPos[2] >= dim[2] - (direction[0] + 1)) { \
...
#define VTKKWRCHelper_InitShader(Event) \
    ...
    emission[3] = SHRT_MAX;                                                 \
    emission[0] = static_cast<unsigned short>((colorTable[0][3*Event.intensity[0]]*emission[3] + 0x7fff)>>(VTKKW_FP_SHIFT)); \
    emission[1] = static_cast<unsigned short>((colorTable[0][3*Event.intensity[0]+1]*emission[3] + 0x7fff)>>(VTKKW_FP_SHIFT)); \
    emission[2] = static_cast<unsigned short>((colorTable[0][3*Event.intensity[0]+2]*emission[3] + 0x7fff)>>(VTKKW_FP_SHIFT)); \
    shader.emissionColor = vec4(static_cast<float>(emission[0]), static_cast<float>(emission[1]), static_cast<float>(emission[2]), static_cast<float>(emission[3])); \
    shader.diffuseColor = shader.emissionColor; \
    shader.specularColor = vec4(static_cast<float>(USHRT_MAX), static_cast<float>(USHRT_MAX), static_cast<float>(USHRT_MAX), shader.emissionColor[3]); \
    shader.ior = 0.5f; \
...
#define VTKKWRCHelper_uniformSampleOneLight(Event) \
    ...
    vec3 localRefRay(Event.refRay.dot(normU), Event.refRay.dot(normV), Event.refRay.dot(Event.norm)); \
    vec3 localScatterPointtoLight(scatterPointToLight.dot(normU), scatterPointToLight.dot(normV), scatterPointToLight.dot(Event.norm)); \
    ...
    vec4 specularColor = shader.specularColor * (microfacepdf + microfacepdf) * std::abs(scatterPointToLight.dot(light.surfaceN)) * weightpdf; \
    imagePtr[0] += Event.lightEnergy * (ambientCoef * shader.emissionColor[0] + diffuseCoef * diffuseColor[0] + specularCoef * specularColor[0]); \
    imagePtr[1] += Event.lightEnergy * (ambientCoef * shader.emissionColor[1] + diffuseCoef * diffuseColor[1] + specularCoef * specularColor[1]); \
    imagePtr[2] += Event.lightEnergy * (ambientCoef * shader.emissionColor[2] + diffuseCoef * diffuseColor[2] + specularCoef * specularColor[2]); \
...
#define VTKKWRCHelper_SampleVolume(MaxStep) \
    ...
    while(cnt < MaxStep && remainEnergy < energy) { \
        ...
        { \
            ...
            endPos[0] = pos[0]; endPos[1] = pos[1]; endPos[2] = pos[2]; \
        } \
        else { \
            initPos[0] = pos[0]; initPos[1] = pos[1]; initPos[2] = pos[2]; \
        } \
        VTKKWRCHelper_CroppingCheckTrilin( pos ); \
        mapper->ShiftVectorDown( pos, spos ); \
        if ( spos[0] != oldSPos[0] || \
        ...
        if(sigmaT > 0.01) { \
            ...
            event.dPos[0] = pos[0]; event.dPos[1] = pos[1]; event.dPos[2] = pos[2]; \
            event.nPos[0] = spos[0]/dim[0];event.nPos[1] = spos[1]/dim[1];event.nPos[2] = spos[2]/dim[2]; \
...
#define VTKKWRCHelper_SampleRay(MaxStep) \
    VTKKWRCHelper_SampleVolume(MaxStep);


template <class T>
void vqFixedPointISOHelperGenerateImageOneTrilin(
    T* dataPtr,
    int threadID,
    int threadCount,
    vtkFixedPointVolumeRayCastMapper* mapper,
...
#define VTKKWRCHelper_SampleVolumeIndependent(MaxStep) \
    ...
    event.dPos[0] = pos[0]; event.dPos[1] = pos[1]; event.dPos[2] = pos[2]; \
     event.nPos[0] = float(spos[0])/float(dim[0]);event.nPos[1] = float(spos[1])/float(dim[1]);event.nPos[2] = float(spos[2])/float(dim[2]); \
...
#define VTKKWRCHelper_InitShaderIndependent(Event) \
    ...
    emission[0] = static_cast<unsigned short>((colorTable[0][3*Event.intensity[0]  ]*emission[3] + 0x7fff)>>(VTKKW_FP_SHIFT)); \
    emission[1] = static_cast<unsigned short>((colorTable[1][3*Event.intensity[1]+1]*emission[3] + 0x7fff)>>(VTKKW_FP_SHIFT)); \
    emission[2] = static_cast<unsigned short>((colorTable[2][3*Event.intensity[2]+2]*emission[3] + 0x7fff)>>(VTKKW_FP_SHIFT)); \
    shader.emissionColor = vec4(float(emission[0]), float(emission[1]), float(emission[2]), float(emission[3])); \
    shader.diffuseColor = shader.emissionColor; \
    shader.specularColor = vec4(static_cast<float>(USHRT_MAX), static_cast<float>(USHRT_MAX), static_cast<float>(USHRT_MAX), shader.emissionColor[3]); \
    shader.ior = 0.5; \
    shader.glossiness = glossinessCoef;
...
#define VTKKWRCHelper_uniformSampleOneLightIndependent(Event) \
    ...
    float lambertianpdf = localReflectAngle * localReflectAngle * INV_PI_F; \
    float cosTheta = std::abs(localRefRay.dot(localScatterPointtoLight)); \
    float microfacepdf = (shader.glossiness) * pow(cosTheta, shader.glossiness) / (2.0f * PI_F * 4.0f * cosTheta); \
    float lightpdf = 1.0f / std::abs(scatterPointToLight.dot(light.surfaceN) * 4.0f); \
    float weightpdf = lightpdf * lightpdf / (lightpdf * lightpdf + lambertianpdf * lambertianpdf); \
    weightpdf = (1.0f - weightpdf); \
    vec4 specularColor = shader.specularColor * (microfacepdf + microfacepdf) * std::abs(scatterPointToLight.dot(light.surfaceN)) * weightpdf; \
    imagePtr[0] = ambientCoef * shader.emissionColor[0] + diffuseCoef * diffuseColor[0] + specularCoef * specularColor[0]; \
    imagePtr[1] = ambientCoef * shader.emissionColor[1] + diffuseCoef * diffuseColor[1] + specularCoef * specularColor[1]; \
    imagePtr[2] = ambientCoef * shader.emissionColor[2] + diffuseCoef * diffuseColor[2] + specularCoef * specularColor[2]; \

template <class T>
void vqFixedPointISOHelperGenerateImageIndependentTrilin(
    T* dataPtr,
    int threadID,
    int threadCount,
    vtkFixedPointVolumeRayCastMapper* mapper,
    ...
    vtkFixedPointVolumeRayCastMapper* mapper)
{
    ...
    else
    {
        ...
        if (mapper->GetCurrentScalars()->GetNumberOfComponents() == 1)
        {
            ...
            {
                vtkTemplateMacro(
                    vqFixedPointISOHelperGenerateImageOneTrilin(
                        static_cast<VTK_TT*>(dataPtr),
                        threadID, threadCount, mapper, vol));
            }
        }
        ...
        else if (vol->GetProperty()->GetIndependentComponents())
        {
            ...
            {
                vtkTemplateMacro(
                    vqFixedPointISOHelperGenerateImageIndependentTrilin(
                        static_cast<VTK_TT*>(dataPtr),
                        threadID, threadCount, mapper, vol));
            }
        }

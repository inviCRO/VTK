...
#ifndef vqVolumeShaderComposer_h
#define vqVolumeShaderComposer_h
...
namespace vqvtkvolume
{
  ...
  //--------------------------------------------------------------------------
  std::string vqComputeClipPositionImplementation(vtkRenderer* vtkNotUsed(ren),
                                                vtkVolumeMapper* vtkNotUsed(mapper),
                                                vtkVolume* vtkNotUsed(vol))
  {
    return std::string("\
      \n  vec4 pos = in_projectionMatrix * in_modelViewMatrix *\
\n             in_InverseOriginMatrix * in_volumeUserMatrix * in_originMatrix * in_translateMatrix *vec4(in_vertexPos.xyz, 1.0);\
      \n  gl_Position = pos;"
    );
  }
  ...
  //--------------------------------------------------------------------------
  std::string vqBaseDeclarationVertex(vtkRenderer* vtkNotUsed(ren),
                                    vtkVolumeMapper* vtkNotUsed(mapper),
                                    vtkVolume* vtkNotUsed(vol))
  {
    return std::string("\
...
");
  ...
  //--------------------------------------------------------------------------
  std::string vqBaseDeclarationFragment(vtkRenderer* vtkNotUsed(ren),
...
\n  g2.z = texture3D(in_volume, vec3(g_dataPos - vec3(0.0, 0.0, in_cellStep[2])))[c];\
  ...
  //--------------------------------------------------------------------------
  std::string vqComputeLightingDeclaration(vtkRenderer* vtkNotUsed(ren),
                                         vtkVolumeMapper* mapper,
                                         vtkVolume* vol,
                                         int noOfComponents,
                                         int independentComponents,
      ...
      for (int i = 0; i < noOfComponents; ++i)
{
  ...
  //--------------------------------------------------------------------------
  std::string vqShadingDeclarationFragment(vtkRenderer* vtkNotUsed(ren),
                                         vtkVolumeMapper* mapper,
                                         vtkVolume* vtkNotUsed(vol))
  {
  ...
  //--------------------------------------------------------------------------
  std::string vqShadingInit(vtkRenderer* vtkNotUsed(ren),
                          vtkVolumeMapper* mapper,
                          vtkVolume* vtkNotUsed(vol),
                        int noOfComponents,
...
\n g_dataPos += g_dirStep;\
  ...
  //--------------------------------------------------------------------------
  std::string vqShadingExit(vtkRenderer* vtkNotUsed(ren),
                          vtkVolumeMapper* mapper,
                          vtkVolume* vtkNotUsed(vol),
                          int noOfComponents,
                          int independentComponents = 0,
  ...
  {
      //JKP couple of strange question here:
      //single component seems to rely on computeColor in the Impl
      //but multi compoment runs computeColor after the Impl
      //First guess is the multi stomps over the Impl work. 
  
      //NOTE the g_srcColor[3] is computed differently in the VQ code (probably wrong)
    vtkOpenGLGPUVolumeRayCastMapper* glMapper =
      vtkOpenGLGPUVolumeRayCastMapper::SafeDownCast(mapper);
    ...
    if (mapper->GetBlendMode() == vtkVolumeMapper::COMPOSITE_BLEND)
    {
        if (compositeMode == VQCompositeMethod::RegularComposite)//AlphaRGBCompositeProjection
        {
            if (noOfComponents > 1 && independentComponents)
            {
                return std::string("\
...
\n  g_fragColor = g_srcColor;"
            ...
            {
                return std::string("\
\n  l_sampledValue.w = 1.0;\
\n vec4 g_srcColor = l_sampledValue;\
\n  g_fragColor.rgb = g_srcColor.rgb * g_srcColor.a;\
\n  g_fragColor.a = g_srcColor.a;"
);
            }
        }
        else if (compositeMode == VQCompositeMethod::FeatureDetection)//AutoRGBCompositeProjection
        {
            if (noOfComponents > 1 && independentComponents)
            {
                return std::string("\
...
\n  g_fragColor.a = g_srcColor.a;"
        ...
        else if (compositeMode == VQCompositeMethod::ColorProjection)
        {
            ...
            {
                return std::string("\
\n  l_sampledValue.rgb = l_sampledValue.rgb * mc_weight;\
\n  g_srcColor = vec4(0);\
\n   g_srcColor.a = 1.0;\
\n  for (int i = 0; i < in_noOfComponents; ++i)\
\n    {\
...
\n  g_fragColor.a = g_srcColor.a;\
  ...
  //--------------------------------------------------------------------------
  std::string vqTerminationInit(vtkRenderer* vtkNotUsed(ren),
                              vtkVolumeMapper* vtkNotUsed(mapper),
                              vtkVolume* vtkNotUsed(vol))
  {
    return std::string("\
...
\n  g_terminatePointMax = length(terminatePoint.xyz - ip_textureCoords.xyz) /\
  ...
  //--------------------------------------------------------------------------
  std::string vqCroppingDeclarationFragment(vtkRenderer* vtkNotUsed(ren),
                                          vtkVolumeMapper* mapper,
                                          vtkVolume* vtkNotUsed(vol))
  {
  ...
  //--------------------------------------------------------------------------
  std::string vqCroppingInit(vtkRenderer* vtkNotUsed(ren),
                           vtkVolumeMapper* mapper,
                           vtkVolume* vtkNotUsed(vol))
  {
      //JKP - Why didn't VQ use this built in cropping code
...
");
  ...
  //--------------------------------------------------------------------------
  std::string vqCroppingImplementation(vtkRenderer* vtkNotUsed(ren),
                                     vtkVolumeMapper* mapper,
                                     vtkVolume* vtkNotUsed(vol))
  {
      //JKP - Why didn't VQ use this built in cropping code
...
#endif // vqVolumeShaderComposer_h
